...
enum T25SchemaGate {
    static func run() async -> Never {
        ...
        do {
            ...
            if fetched.providerName != status.providerName { mismatches.append("providerName") }
            if fetched.providerDisplayName != status.providerDisplayName { mismatches.append("providerDisplayName") }
            if fetched.ok != status.ok { mismatches.append("ok") }
            if fetched.errorMessage != status.errorMessage { mismatches.append("errorMessage") }
            if fetched.windows != status.windows { mismatches.append("windows") }
            if fetched.data != status.data { mismatches.append("data") }
            if fetched.observedAt != status.observedAt { mismatches.append("observedAt") }
            if fetched.snapshotUpdatedAt != status.snapshotUpdatedAt { mismatches.append("snapshotUpdatedAt") }
