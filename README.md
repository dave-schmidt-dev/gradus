...
- Claude: Claude Code authenticated with `claude auth login`; Gradus reads the OAuth access token read-only from the macOS Keychain (service `Claude Code-credentials`) and sends it only to Anthropic's usage endpoint. On a stale access token with a verified future refresh expiry, the credential-aware producer makes at most one bounded call to the fixed `claude-headless --verify-auth` wrapper; only its exact positive result followed by a fresh Keychain expiry permits a usage request. Busy, failed, timed-out, or unchanged recovery retains the explicit stale state and cached reading. Consumer paths never invoke the wrapper, and Gradus never handles credential material or promises automatic login. Absent or malformed refresh metadata, or no valid cached reading, reports a no-values unavailable state. The canonical provider remains `ok: false` in all of these cases so routers fail closed. Run `claude auth login` only when the refresh grant is known to have expired.
...
- Claude's interactive provider reads Claude Code's `claudeAiOauth.accessToken` from the macOS Keychain item `Claude Code-credentials` without persisting or logging credential material, then calls Anthropic's structured `/api/oauth/usage` endpoint. The credential-aware producer may make one bounded `claude-headless --verify-auth` call only for a stale access token with verified future refresh expiry; it accepts only the wrapper's exact positive result and a subsequent fresh Keychain expiry. Busy, failed, timed-out, or unchanged recovery retains `Claude Code credential is stale; showing cached values`; no consumer path invokes the wrapper and this is not automatic login. It maps `five_hour`, `seven_day`, and `seven_day_opus` utilization and reset fields into remaining-capacity windows. Headless paths report `auth required: no cached credentials`; an expired refresh grant reports `Claude Code session expired: run \`claude auth login\``. Unknown refresh expiry produces `Claude Code refresh metadata unavailable; provider not queried`, and no valid prior produces `Claude Code usage unavailable; no cached values`. Neither no-values state carries an observation. Each outcome remains `ok: false`, so routers fail closed.
...
- **Claude usage requires an explicit credential-aware producer probe.** Gradus reads Claude Code's OAuth token from Keychain and calls Anthropic's structured usage endpoint; consumer surfaces do not access Keychain or invoke recovery. Probes run at most every 10 minutes and back off for one hour after HTTP 429. Only an expired access token with verified future refresh expiry may trigger one bounded fixed-wrapper verification; the producer reloads the Keychain and proceeds only with a fresh access expiry. Busy, failed, timed-out, or unchanged recovery retains valid cached windows for 24 hours while remaining `ok: false`; this does not perform automatic login. Otherwise Gradus exposes no values. Canonical v2 remains `ok: false` in each unavailable case, so routers fail closed. If the refresh grant is known to be expired, authenticate with `claude auth login` and run `--refresh-snapshot`.
