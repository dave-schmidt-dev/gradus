...
- Claude's interactive provider reads Claude Code's `claudeAiOauth.accessToken` from the macOS Keychain item `Claude Code-credentials` without persisting, refreshing, logging, or launching Claude Code, then calls Anthropic's structured `/api/oauth/usage` endpoint. It maps `five_hour`, `seven_day`, and `seven_day_opus` utilization and reset fields into remaining-capacity windows. Headless paths report `auth required: no cached credentials`; an expired refresh grant reports `Claude Code session expired: run \`claude auth login\``. A stale access token with verified future refresh expiry produces `Claude Code credential is stale; showing cached values` and carries only a valid prior reading for 24 hours; unknown refresh expiry produces `Claude Code refresh metadata unavailable; provider not queried`, and no valid prior produces `Claude Code usage unavailable; no cached values`. Neither no-values state carries an observation. Each outcome remains `ok: false`, so routers fail closed.
...
- **Claude usage requires an explicit credential-aware producer probe.** Gradus reads Claude Code's OAuth token from Keychain and calls Anthropic's structured usage endpoint; consumer surfaces do not access Keychain, and Gradus never refreshes, writes, or launches Claude Code with the credential. Probes run at most every 10 minutes and back off for one hour after HTTP 429. Only an expired access token with verified future refresh expiry may retain valid cached windows for 24 hours; otherwise Gradus exposes no values. Canonical v2 remains `ok: false` in each unavailable case, so routers fail closed. If the refresh grant is known to be expired, authenticate with `claude auth login` and run `--refresh-snapshot`.
