...
- **GradusWidget** — one `systemSmall` WidgetKit extension embedded in GradusiOS. The main app writes an atomic, allowlisted JSON projection to `group.com.zerodelta.gradus`; the extension reads only that file and performs no network, CloudKit, notification, or credential work. It shows the fixed most-urgent provider/window, remaining percentage, signal rail, reset time, and age of the phone's last successful sync. Missing or invalid data asks the user to open Gradus. Tapping opens the app dashboard. Sample and UI-test fixtures never publish widget data.
